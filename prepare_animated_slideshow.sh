...
# Get list of already processed files (basenames only)
OLD_FILES=()
while IFS= read -r file; do
  OLD_FILES+=("$(basename "$file")")
done < <(find "$slideshow" -name "*.gif")

$VERBOSE && echo "Existing gifs:"
$VERBOSE && printf "%s\n" "${OLD_FILES[@]}"

# Collect all source files (basenames only)
NEW_FILES=()
let i=0; let k=0
for dir in ${dirs[@]}
do
  ...
  do
    ...
    NEW_FILES+=("$basename_file")
    
    # Skip if already processed
    [ -e "$slideshow$basename_file" ] && continue
...
# Find files that exist in OLD_FILES but not in NEW_FILES (orphaned processed files)
ORPHANED=()
...
$VERBOSE && echo "New set: ${NEW_FILES[@]}"
$VERBOSE && echo "Orphaned files to delete: ${ORPHANED[@]}"
